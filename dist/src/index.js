...
Page({
  ...
  initData(){
    ...
    positioning_controls.position.top = deviceInfo.windowHeight - (240 + 58)
    util.loactionAddress(this.initData).then(res => {
      this.setData({
  ...
  peopleLookingForAcar: function(e){
    ...
    return_controls.position.top = win_height - (240 + 46)
  ...
  controltap(e) {
    ...
    {
    case 1:
      animation.bottom(-240).opacity(0).step()
      this.setData({
        switch_type: 'index',
  ...
  postTravelPrice: function(role, seat){
    ...
    driver_api.postTravelPrice({
      ...
      json.data.priceList && json.data.priceList.map(res => {
        price.push(role == 'people' ? res.price + '元' : res.price + '元/座')
  ...
  creatTravel(){
    ...
    if(!moment(time).isAfter(moment().toDate().pattern('yyyy-MM-dd HH:mm:ss'))){
      wx.showModal({
        title: '提示',
        content: '选择时间应大于当前时间',
        showCancel: false
